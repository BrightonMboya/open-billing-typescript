...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.SubscriptionCancelSubscriptionRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/subscription/{subscription_id}/cancel")(pathParams);
  ...
  >(
    M.json(
      200,
      operations.SubscriptionCancelSubscriptionResponseBody$inboundSchema,
    ),
    M.jsonErr(
      400,
      errors.SubscriptionCancelSubscriptionResponseBody$inboundSchema,
    ),
    M.jsonErr(
      404,
      errors
